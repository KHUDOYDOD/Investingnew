...
-- =====================================================
-- ПОЛНЫЙ ЭКСПОРТ БАЗЫ ДАННЫХ INVESTPRO
-- =====================================================
-- Этот файл содержит полную структуру и данные базы данных
-- Создан: $(date '+%Y-%m-%d %H:%M:%S')
-- Для восстановления запустите этот файл в PostgreSQL
...
-- Удаляем все существующие таблицы в правильном порядке
...
INSERT INTO users (
    ...
    '$2b$10$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj/VcSAg/9qm', 
    '+7900000001', 
    'Russia', 
    true, 
    true, 
...
-- Тестовый пользователь (пароль: X12345x)
INSERT INTO users (
    id, login, email, full_name, password_hash, phone, country, 
    is_verified, is_active, balance, total_invested, total_earned, 
    referral_code, role_id, avatar_url, status, created_at, updated_at
...
('crypto', 'USDT TON', '{
    "address": "EQD6eUEZX8uSYEAcnZOD7N6vGj6dYQJ9lXz8vHx0Y9Q7Hkz6", 
    "network": "TON", 
    "currency": "USDT",
...
-- Отзывы
INSERT INTO testimonials (name, position, content, rating, avatar_url, is_active) VALUES
('Алексей Иванов', 'Предприниматель', 'Отличная платформа для инвестиций. Уже полгода получаю стабильную прибыль!', 5, '/avatars/user1.jpg', true),
('Мария Петрова', 'Финансовый аналитик', 'Профессиональный подход к инвестированию. Рекомендую всем коллегам.', 5, '/avatars/user2.jpg', true),
('Дмитрий Сидоров', 'IT-директор', 'Прозрачные условия и быстрые выплаты. Именно то, что нужно для долгосрочных инвестиций.', 5, '/avatars/user3.jpg', true)
ON CONFLICT DO NOTHING;
...
-- Этот файл содержит полную структуру базы данных и все данные
-- Для восстановления просто запустите этот файл в PostgreSQL
-- 
-- Доступы:
-- Администратор: admin@example.com / X12345x
-- Тестовый пользователь: test@example.com / X12345x
--
